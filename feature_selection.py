...
if __name__ == '__main__':
    ...
    # %% Load dataset
    data = pd.read_csv("./EMG_ANN_gridsearch/LIH/Processed_data_final_ver2.csv")
    
    feature_onset = ['zc', 'ssc', 'wl']
    feature_selection_set = ["mav", "rms", "var"]
    channel_list = ['ch1', 'ch2', 'ch3']
    
    label_map = {'rest': 0 , 'four':1, 'good':2, 
                'gun': 3, 'okay':4, 'one':5, 
                'paper':6, 'rock': 7, 'scissor':8, 
                'three':9}


    for number_of_combination in range(0, 4):
        
        for comb in tqdm(combinations(feature_selection_set, number_of_combination)):
            
            tmp_feature_list = feature_onset + list(comb)
            
            feature_selected = []

            for channel in channel_list:
                for feature in tmp_feature_list:
                    feature_selected += [channel + "_" + feature]
            
            
            
            X_data = data.drop(["label", "label2"], axis=1)
            y_data = data["label2"].map(label_map)
            ...
            X_data = X_data[feature_selected].values
            ...
            for num_layers in HP_NUM_LAYERS.domain.values:
                for num_units in HP_NUM_UNITS.domain.values:
                    for dropout_rate in HP_DROPOUT.domain.values:
                        ...
                        run_name = "run-" + '_'.join(tmp_feature_list) + "-%d" % session_num
            ...
            final_model.save('./final_model' + '_'.join(tmp_feature_list) + '.h5')

            # load saved model and calcuate accuracy with test datasets
            saved_model =tf.keras.models.load_model('./final_model' + '_'.join(tmp_feature_list) + '.h5')
