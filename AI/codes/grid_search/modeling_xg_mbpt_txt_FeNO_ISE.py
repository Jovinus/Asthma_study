...
# %%
feature_mask = baseline_txt + mbpt_txt + ['FeNO', 'ISE_Eos']
        ...
        for hyper_labmda in tqdm(hyper_param_labmda, desc= 'lambda'):
            ...
            for hyper_gamma in tqdm(hyper_param_gamma, desc= 'gamma'):
                ...
                for train_index, validation_index in skf.split(train_set, train_set['Asthma']):
                        ...
                        test_ppv_list.append(ppv)
                        test_npv_list.append(npv)
                        test_sensitivity_list.append(sensitivity)
                        test_specificity_list.append(specificity)
                        test_accuracy_list.append(accuracy)
                        test_f1_list.append(f1)
                        test_threshold_list.append(threshold_of_interest)
                ...
                result = {'max_depth': hyper_depth, 
                          ...
                          'test_ppv':test_ppv_list,
                          'test_npv':test_npv_list,
                          'test_sensitivity':test_sensitivity_list,
                          'test_specificity':test_specificity_list,
                          'test_accuracy':test_accuracy_list,
                          'test_f1':test_f1_list,
                          'test_threshold':test_threshold_list
                          }
                
                results["depth_" + str(hyper_depth) + "_lr_" + str(hyper_lr) + "_lambda_" + str(hyper_labmda) + "_gamma_" + str(hyper_gamma)] = result
# %%
with open('./result/results_mbpt_txt_FeNO_ISE.pickle', 'wb') as file_nm:
